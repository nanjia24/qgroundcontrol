...
## 2026-09-02 Windows operator-workflow final safety closure

- Final review found two command-target safety gaps after `9d18263ef`: left-handed non-auto-centering Quad control restored the actual right-hand throttle stick to 50%, and multi-vehicle confirmation retained only the active Vehicle while execution reread a mutable live selection.
- Virtual joystick policy now derives positive range, auto-centering, reset, calibration, and resize behavior from the handedness-selected throttle stick. Sending remains gated through restore, and the neutral reset covers left/right-handed, auto-center on/off, and Quad/Rover profiles.
- Multi-vehicle Guided Actions now snapshot the selected system-ID set and Vehicle objects at confirmation, require an order-independent exact ID match at execution, then resolve the matching current model objects before dispatch. Changed, added, removed, or duplicated selections fail closed, while a reconnected wrapper with the same system ID receives the command instead of the stale object.
- A component-level QML gate now instantiates the production `VirtualJoystick`, `GuidedActionsController`, and `GuidedActionConfirm`. It covers actual handedness wiring, hidden/restore sending, selection drift, same-ID live-object replacement, matching execution, legacy 2/4-argument calls, missing-snapshot rejection, and map-indicator exactly-once behavior.
- The final Windows Debug incremental build linked `Debug/QGroundControl.exe`. QML Quick Test passed 20/20, and the component test plus nine Hybrid/airframe/joystick/APM focused tests passed 10/10 in 76.91 seconds. FlyView and QmlControls qmllint targets exited 0 with existing unrelated diagnostics retained; the two new Loader-context warnings were eliminated. An isolated offscreen `--allow-multiple --swrast` smoke instance remained alive for 15 seconds without a QML-load or fatal match.
- The component test also exposed and closed missing fixture context for the production fade lifecycle, static `hideTrigger` assignments that cleared fresh target snapshots, local selection-model shadowing, and unstable cross-component QObject-wrapper identity.
- Final independent read-only review found no Critical or Important issue in the current-object dispatch, confirmation fade lifecycle, virtual-joystick policy, test registration, or state evidence.
- This closure changes only Windows desktop QGC. PX4, MAVLink XML/release, and firmware configuration remain unchanged; physical-aircraft acceptance is still required.
...
- The current QGC protocol pin is protected tag `qgc-hybrid-rover-tuning-v1.16.1-r2`, peeled commit `ec506d609e775035b7c8ed37f09ef05774409281`, protected by active GitHub ruleset `22434667`. It retains type 200, command 50000, status 60000, the four Rover tuning messages, APM definitions, and the Storm32 exclusion while adding symbolic HX65 sensor/backend enums without changing message 60000 layout or CRC.
- Static vehicle routing remains exclusively `HEARTBEAT.type == MAV_TYPE_QUAD_ROVER == 200`; `HYBR_QUAD_ROV` has been removed from the PX4 feature branch and must not be reintroduced in QGC.
- The merged UI route is a dedicated Quad-Rover tuning container: `Multirotor` retains the standard PX4 Copter pages, while `Rover` exposes Rate, Attitude, Velocity, and Path Tracking. Type 22 remains on the ordinary VTOL page.
- Rover stream control uses the actual primary link's MAVLink output flag, link-pinned command-511 ACK matching, one ambiguous request per component/link, late-ACK quarantine, primary-link migration, and explicit restore debt. `MAV_PROTO_VER` and `HYBR_QUAD_ROV` are not classification or link-capability inputs.
- Physical acceptance is split into three non-actuating stages: static page/identity inspection needs no battery; USB MAVLink2/inactive-state inspection needs no battery; active controller waveforms require a battery, removed propellers, raised wheels, an available emergency stop, and explicit user coordination. QGC must never auto-arm or command motors during these checks.
- The Windows Debug configuration resolves VS2022 x64 `cl/link/lib`, keeps the ArduPilot dialect and plugin enabled, selects `qgc_hybrid` MAVLink 2, and checks out immutable tag `qgc-hybrid-rover-tuning-v1.16.1-r2` at `ec506d609e775035b7c8ed37f09ef05774409281`.
- The r2 dependency update regenerated QGC C headers with `HYBRID_VEHICLE_SENSOR_HX65=4` and `HYBRID_VEHICLE_ACTUATOR_HX65=2`; the full Windows Debug target linked successfully, all seven Hybrid MAVLink contract cases passed, and `HybridVehicleStateTest` passed in the established Qt/GStreamer offscreen environment.
- Final post-fix validation passed: contract/UI/Fact/QML group 12/12, command/request/link/Vehicle telemetry group 7/7, and Hybrid/Mission group 7/7. The final incremental build exited 0 and linked `build/windows-debug-rover-tuning/Debug/QGroundControl.exe`; the exact executable survived a new 15-second offscreen smoke with no QML-load or fatal match.
- `QGroundControlControlsModule_qmllint` and a direct lint of every changed PX4 tuning QML file exited 0. The aggregate PX4 module target remains red only on pre-existing generated `PowerComponent.qml` duplicate IDs; no generated file was edited to hide that unrelated baseline problem.
- Final review closed two reliability findings: a missing command-511 ACK now quarantines the ambiguous `(component, link)` key for one additional link-specific ACK window instead of the full connection lifetime, and ordinary per-stream timestamp reordering is dropped without resetting unrelated curves. A confirmed Hybrid reboot event resets all Rover tuning clocks, after which low boot-era timestamps are accepted. The focused post-fix build and four regression tests exited 0.
...
- The wireless fix refreshes the current primary-link channel synchronously before the Rover gate and adds opt-in `Vehicle.PIDTuningTelemetry` diagnostics for page/mode, link/channel, output version, message interval, target, generation, and ACK outcome.
- Live wireless diagnosis exposed a second runtime race: synchronous Rover QML construction could occupy the main thread for about 2.96 seconds immediately after the old page started its message-interval restore, allowing the generic 3-second command timeout to run before the returned ACK was processed. PID telemetry pages now use guarded ownership leases and defer destruction-time release; a replacement page supersedes the old lease before any restore command can race its construction.
- Wireless target acceptance is complete. On the Tailscale/UDP/mavlink-router route, the exact isolated QGC logged `ModeRoverRate`, MAVLink2 on UDP channel 1, a `60100/20000` request, and its matching accepted ACK; the operator confirmed that the Rover Rate waveform was visible.
