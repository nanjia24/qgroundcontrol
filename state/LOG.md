...
- Design self-review added the missing ACK-routing boundary: the generic queue matches a pending entry by command and sending component only, so a detached controller additionally validates the active vehicle system id, original autopilot component, QGC ACK address, and exact optional `uint32` transition sequence. Direct terminal ACKs before progress remain on the ordinary queue path.
