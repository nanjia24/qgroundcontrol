...
void RoverTuningFactGroupTest::_timestampLifecycle_test()
{
    ...
    QCOMPARE(group.attitudeTimestamp()->rawValue().toULongLong(), 0ULL);
    QCOMPARE(group.velocityTimestamp()->rawValue().toULongLong(), 0ULL);
    QCOMPARE(group.positionTimestamp()->rawValue().toULongLong(), 0ULL);
    ...
    QVERIFY(!group.rateAvailable());
    QCOMPARE(group.rateResetCounter(), 2U);
    QCOMPARE(group.attitudeResetCounter(), 2U);
    QCOMPARE(group.velocityResetCounter(), 2U);
    QCOMPARE(group.positionResetCounter(), 2U);
}
