...
private slots:
    ...
    void _miniRoverVehicleTypeGate();
...
void VehiclePIDTuningTelemetryTest::_miniRoverVehicleTypeGate()
{
    {
        Vehicle fixedRotorVehicle(MAV_AUTOPILOT_PX4, MAV_TYPE_VTOL_FIXEDROTOR);
        QVERIFY(fixedRotorVehicle.miniRover());
    }

    static constexpr std::array<MAV_TYPE, 4> otherVehicleTypes{
    ...
    for (const MAV_TYPE vehicleType : otherVehicleTypes) {
        ...
        QVERIFY(!otherVehicle.miniRover());
    }
}
...
void VehiclePIDTuningTelemetryTest::_messageIntervalRequestsAreSerializedAcrossCallers()
{
    ...
    mockLink()->setMessageIntervalResponseEnabled(true);
    mockLink()->sendUnexpectedCommandAck(MAV_CMD_SET_MESSAGE_INTERVAL, MAV_RESULT_ACCEPTED);
    QTRY_COMPARE_WITH_TIMEOUT(_matchingRequestCount(MAVLINK_MSG_ID_HIGHRES_IMU, 50000), 1,
                              TestTimeout::shortMs());
