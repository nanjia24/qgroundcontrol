...
- Mini Rover PX4 firmware uses the global `MAV_PROTO_VER` parameter, not an instance-specific `MAV_x_PROTO_VER`; value `2` forces MAVLink 2 and requires a flight-controller restart.
- PX4 Rover metadata includes broad ranges with fine increments, for example `RO_YAW_RATE_LIM` uses `0..10000` and increment `0.01`; increment describes parameter precision, not a safe UI major-tick interval.
- On this Windows GUI build, redirected QGC category logs require `QT_FORCE_STDERR_LOGGING=1`; `--log-output` alone only produced the QML-debugging notice without a debugger.
- PX4 firmware `d656c31dd794e3cfec992c63af789deb012dce83` retains an unbounded, unsynchronized `_subscribe_to_stream` handoff in `Mavlink::configure_stream_threadsafe()`. USB evidence shows request ACKs delayed about 40 seconds and a standard message-281 interval command still unacknowledged after 60 seconds; do not attribute the preceding Rally timeout to Rover messages.
...
- `MAV_CMD_SET_MESSAGE_INTERVAL` ACKs do not identify the requested message ID. PID tuning transitions must keep only one such command outstanding per generation, retain restore debt until an accepted ACK, and finish old cleanup before applying a new stream.
- `MAV_CMD_SET_MESSAGE_INTERVAL` must be single-flight per `(Vehicle, component, resolved link)` across all callers, not only inside one feature. Link-pinned requests must match the incoming ACK link, and failed old-primary restore debt must remain link-owned without blocking a new primary. On terminal timeout, fail that key's queued entries and quarantine it until one late ACK is consumed or the link disconnects/dies; never advance another `param1` from an ambiguous late ACK. Use the 60-second timeout only for PX4 direct USB, retaining the normal 1200 ms policy elsewhere.
- Observe every pinned `MAV_CMD_SET_MESSAGE_INTERVAL` link from enqueue time and fail that link's active/waiting entries immediately on disconnect; a stale USB entry must never hold backup-primary configuration for the 60-second USB timeout. Queue result handlers are synchronously reentrant, so remove the current stream item before invoking a send callback that may abort or clear its state machine.
- Source-timestamp PID charts may batch rendering work, but every distinct PX4 `time_usec` must enqueue an immutable timestamp/value snapshot. A boolean "pending latest sample" flag violates the telemetry contract and can lose 50 Hz peaks under normal timer jitter or USB burst delivery.
- A `PX4TuningComponent` loaded inside another tuning component's Loader must explicitly bridge `availableWidth` and `availableHeight` from that Loader. Do not rewrite the shared base to use `parent.available*`; its top-level `SetupPage` loading path receives those names through context instead.
- Before USB acceptance, verify the running QGC process executable path matches the task-local build and close other QGC instances; an installed or different-worktree process can retain the serial port and invalidate observations.
- Never feed a Fact increment directly into a QML Repeater-backed slider's major-tick count without bounding `(max-min)/step`; preserve edit precision separately from display tick density and regression-test broad-range metadata.
