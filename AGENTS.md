...
- Do not assume `ninja -t clean <object>` removes only that object; order-only dependencies can expand the clean to thousands of generated outputs. Prefer exact validated artifact removal, or use a fresh/recoverable build tree when exact removal is unavailable.
- Confirmed Guided Actions must retain the initiating `Vehicle`, cancel both delayed and visible confirmation when the active vehicle changes, and reject execution if the initiating vehicle is no longer active.
- Virtual joystick sending must be disabled while its item is hidden or a vehicle/shape reset is pending. Position resets performed while hidden must explicitly synchronize cached axes before sending resumes.
- A QGC-owned PX4 airframe overlay must replace same-name group metadata as well as its declared autostart IDs; otherwise a firmware cache can preserve stale group imagery while showing the new airframe entry.
- Virtual-joystick throttle policy must be applied to the actual handedness-selected throttle stick. Positive-range, auto-center, restore, calibration, and resize paths must all use that same stick and keep sending gated until its cached axes are synchronized.
- Confirmed multi-vehicle Guided Actions must snapshot the selected system-ID set plus its Vehicle objects and fail closed if the live ID set differs before execution; after validation, dispatch through the matching current model objects so a reconnected same-ID wrapper cannot leave a stale target. Retaining only the active Vehicle does not protect the selected fleet, and QObject wrapper identity is not stable across QML component property boundaries.
- `MAV_CMD_SET_MESSAGE_INTERVAL` ACKs do not echo the requested message ID. Serialize them per `(target component, physical link)`, bind ACKs to the sending link, and quarantine a timed-out key for a bounded link-specific ACK window; an unbounded tombstone can permanently disable every interval user after one lost ACK.
